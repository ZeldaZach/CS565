...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, starting_game_state):
        ...
        self.startingPosition = starting_game_state.getPacmanPosition()
    ...
    def getSuccessors(self, state):
        ...
        for action in [
            ...
            # Get current state's position
            state_x, state_y = state[0]

            # Next direction to move based on the action
            dx, dy = Actions.directionToVector(action)
            next_x, next_y = int(state_x + dx), int(state_y + dy)

            # See if this coordinate is a wall
            hits_wall = self.walls[next_x][next_y]
            if not hits_wall:
                next_state = next_x, next_y

                next_corners_visited = list(state[1])

                corner_visited_index = self.__corner_to_modify(next_state)
                if next_state in self.corners:
                    next_corners_visited[corner_visited_index] = True

                successors.append(((next_state, next_corners_visited), action, 1))

        self._expanded += 1  # DO NOT CHANGE
        return successors

    @staticmethod
    def __corner_to_modify(state):
        if state[0] == 1:
            if state[1] == 1:
                return 0
            else:
...
def cornersHeuristic(state, problem):
    ...
    for index, corner in enumerate(corners):
        # If we haven't been to this corner yet in the state corner list
        if not state[1][index]:
            # See how far away we are manhattan wise
            manhattan_dist = util.manhattanDistance(state[0], corner)

            # If we're further than expected, up our heuristic value
            if manhattan_dist > heuristic_dist:
